...
function ContentBox() {
  ...
  const handleSummarize = async () => {
    ...
    try {
      const wordLimit = parseInt(maxWordLimit, 10);
      
      if (isNaN(wordLimit) || wordLimit <= 0) {
        alert('Please enter a valid word limit (minimum 50 words).');
        setLoading(false);
        return;
      }

      const formData = new FormData();
      formData.append('words_limit', wordLimit);
      ...
      if (selectedMethod === 'file' && file) {
        ...
        const response = await axios.post(
          'http://localhost:9000/summarize/file',
          formData,
          {
            headers: {
        ...
        const response = await axios.post('http://localhost:9000/summarize/text', formData);
        setSummary(response.data.summary);
      } else if (selectedMethod === 'url' && content) {
        formData.append('url', content);
        const response = await axios.post('http://localhost:9000/summarize/url', formData);
        setSummary(response.data.summary);
      } else {
